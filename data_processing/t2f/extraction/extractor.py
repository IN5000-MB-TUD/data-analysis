...
def extract_single_series_features(record: np.array, sensors_name: list):
    ...
    features_single = {
        "single__{}".format(k): val for k, val in features_extracted.items()
    }

    return features_single


def extract_single_series_features_batch(
    ts_list: np.array, batch_size: int = -1, pid: int = 1
):
    """Extract features for each signal in each time series in batch mode"""
    sensors_name = [str(i) for i in range(ts_list[0].shape[0])]
    ...
    for i in range(num_batch):
        ...
        for k, val in features_extracted.items():
            ...
            for stat_k, stat_val in val.items():
                df_features[pos][f"{name}__{stat_k}"] = stat_val
...
def extract_pair_series_features(mts: np.array):
    ...
    for i, j in combs:
        ...
        mts_i = [
            (int(mts_i_time.timestamp()), mts_i_value)
            for mts_i_time, mts_i_value in mts_i
        ]
        mts_j = [
            (int(mts_j_time.timestamp()), mts_j_value)
        ...
        feature = {
            "pair__{}__{}__{}".format(k, i, j): val for k, val in feature.items()
        }
...
def feature_extraction_simple(
    ts_list_pairs: (list, np.array),
    ts_list_single: (list, np.array),
    batch_size: int = -1,
    pid: int = 1,
):
    ...
    for ts_record in ts_list_pairs:
    ...
    df_single_features = extract_single_series_features_batch(
        ts_list_single, batch_size=batch_size, pid=pid
    )
...
def feature_extraction(
    ...
    for i, size in enumerate(balance_job):
        list_arguments.append(
            (
                ts_list[index : index + size],
                ts_list_single[index : index + size],
                batch_size,
                i,
            )
        )
