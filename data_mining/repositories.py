...
from connection import mo, github_api_client
...
def update_github_repository_data(repository_owner, repository_name):
    """
    Retrieve and update the data from the GitHub API for the given repository.

    :param repository_owner: The owner of the repository.
    :param repository_name: The name of the repository.
    :return: True if the data have been updates. False otherwise.
    """
    ...
    if (
        ...
        and (datetime.now(tz=utc) - repository_db_record["metadata"]["modified"]).days
        < 1
    ):
        log.info(
    ...
    repository_data = {
        ...
        "age": (
            datetime.now(tz=utc)
            - datetime.strptime(github_api_data["created_at"], DATE_FORMAT).replace(
                tzinfo=utc
            )
        ).total_seconds(),
        "metadata": {
