...
REPOSITORIES = [
    "saltstack/salt",  # Steep
    "SuperEvilMegacorp/vainglory-assets",  # Semi-Shallow
    "uber/storagetapper",  # Shallow
]
REPOSITORY_CLUSTERS = [0, 1, 2]
...
METRICS_IMPORTANCE = {
    ...
    "cluster_1": {
        ...
        "forks": "releases",
        "pull_requests": "forks",
        "workflows": "pull_requests",
        "releases": "stargazers",
        "size": "commits",
    },
    "cluster_2": {
        "stargazers": "workflows",
        "issues": "pull_requests",
        "commits": "size",
        "contributors": "commits",
        "deployments": "stargazers",
        "forks": "pull_requests",
...
if __name__ == "__main__":
    ...
    for repository_idx, repository_full_name in enumerate(REPOSITORIES):
        ...
        metrics_importance = METRICS_IMPORTANCE[f"cluster_{repository_cluster}"]
        log.info(
        ...
        for n in N_MONTHS:
            for metrics_full in [True, False]:
                metrics_full_key = (
                    "important_metric" if not metrics_full else "full_metrics"
                )

                log.info(f"Evaluating {n} months training for {metrics_full_key}")
                total_predictions = 0
                ...
                for target_metric in METRICS:
                    ...
                    # Keep most important metric only if computing for that
                    if not metrics_full:
                        df_training = df_training[
                            ["ds", "unique_id", "y", metrics_importance[target_metric]]
                        ]
                        df_validation = df_validation[
                            ["ds", "unique_id", metrics_importance[target_metric]]
                        ]
                ...
                if total_predictions > 0:
                    ...
                    n_months_accuracy[repository_full_name][metrics_full_key][
                        f"{n}_months"
                    ] = {
    ...
    for repository, metrics_selection in n_months_accuracy.items():
        for (
            metrics_selection_key,
            metrics_selection_values,
        ) in metrics_selection.items():
            if metrics_selection_key == "cluster":
                continue

            for month_key, month_val in metrics_selection_values.items():
                if month_key not in average_scores[metrics_selection_key]:
                    average_scores[metrics_selection_key][month_key] = {
                        "performance": 0,
                        "deviation": 0,
                        "r2": 0,
                        "precision": 0,
                ...
                average_scores[metrics_selection_key][month_key][
                    "performance"
                ] += month_val["performance"]
                average_scores[metrics_selection_key][month_key][
                    "deviation"
                ] += month_val["deviation"]
                average_scores[metrics_selection_key][month_key]["r2"] += min(
                    1, max(0, month_val["r2"])
                )
                average_scores[metrics_selection_key][month_key][
                    "precision"
                ] += month_val["precision"]
                average_scores[metrics_selection_key][month_key]["recall"] += month_val[
                    "recall"
                ]
                average_scores[metrics_selection_key][month_key][
                    "hausdorff"
                ...
                average_scores[metrics_selection_key][month_key][
                    "random_index"
                ] += month_val["random_index"]

    total_projects = len(REPOSITORIES)
    for metrics_selection_key, metrics_selection_values in average_scores.items():
        for month_key in metrics_selection_values.keys():
            average_scores[metrics_selection_key][month_key][
                "performance"
            ] /= total_projects
            average_scores[metrics_selection_key][month_key][
            ...
            average_scores[metrics_selection_key][month_key]["r2"] /= total_projects
            average_scores[metrics_selection_key][month_key][
                "precision"
            ] /= total_projects
            average_scores[metrics_selection_key][month_key]["recall"] /= total_projects
            average_scores[metrics_selection_key][month_key][
                "hausdorff"
            ] /= total_projects
            average_scores[metrics_selection_key][month_key][
    ...
    # Store results
    with open("../data/features_importance.json", "w") as outfile:
